...
class MemoryPageMixin:
    def build_memory_page(self, parent):
        self.page_header(parent, t("memory.page_title"), t("memory.page_subtitle"))
        ...
        ctk.CTkLabel(tip_card, text=t("memory.tip_title"), font=("Microsoft YaHei UI", 14, "bold"), text_color=COL_TEXT).grid(row=0, column=0, padx=18, pady=(14, 2), sticky="w")
        ctk.CTkLabel(tip_card, text=t("memory.tip_body"), font=("Microsoft YaHei UI", 11), text_color=COL_TEXT_MUTED, wraplength=980, justify="left").grid(row=1, column=0, padx=18, pady=(0, 14), sticky="w")
        ...
        buttons = [
            (t("memory.add_rule"), None, self.add_memory_rule_dialog),
            (t("memory.edit_selected"), None, self.edit_selected_memory_rule),
            (t("memory.delete_selected"), COL_DANGER, self.delete_selected_memory_rules),
            (t("memory.save"), COL_OK, self.save_memory_panel),
            (t("memory.reload"), "#6b7280", self.load_memory_panel),
        ]
        ...
        self.memory_count_var = tk.StringVar(value=t("memory.count", count=0))
        self.memory_status_var = tk.StringVar(value=t("memory.status_loaded"))

        ctk.CTkLabel(status_box, text=t("memory.status_title"), font=("Microsoft YaHei UI", 10), text_color=COL_TEXT_MUTED).pack(anchor="w", padx=14, pady=(10, 0))
        ctk.CTkLabel(status_box, textvariable=self.memory_count_var, font=("Microsoft YaHei UI", 14, "bold"), text_color=COL_TEXT).pack(anchor="w", padx=14, pady=(2, 0))
        ctk.CTkLabel(status_box, textvariable=self.memory_status_var, font=("Microsoft YaHei UI", 11), text_color=COL_TEXT_MUTED, wraplength=180, justify="left").pack(anchor="w", padx=14, pady=(2, 10))
        ...
        ctk.CTkLabel(table_header, text=t("memory.rules_title"), font=("Microsoft YaHei UI", 14, "bold"), text_color=COL_TEXT).grid(row=0, column=0, sticky="w")
        ctk.CTkLabel(table_header, text=t("memory.rules_subtitle"), font=("Microsoft YaHei UI", 11), text_color=COL_TEXT_MUTED).grid(row=1, column=0, pady=(2, 0), sticky="w")
        ...
        headings = {"match": t("memory.col_keyword"), "category": t("memory.col_category"), "note": t("memory.col_note")}
        ...
        self.memory_tree.bind("<Double-1>", lambda event: self.edit_selected_memory_rule())
        self.load_memory_panel()

    def _setup_ttk_style(self):
        style = ttk.Style()
        style.theme_use("default")
        style.configure("Treeview", background="#ffffff", foreground="#111827", rowheight=32, fieldbackground="#ffffff", borderwidth=0, relief="flat", font=("Microsoft YaHei UI", 10))
        style.configure("Treeview.Heading", background="#f8fafc", foreground="#111827", font=("Microsoft YaHei UI", 10, "bold"), relief="flat", borderwidth=0)
    ...
    def load_memory_panel(self):
        ...
        try:
            ...
            self._refresh_memory_status(t("memory.loaded_from_file"))
        except Exception as exc:
            messagebox.showerror(t("memory.load_failed_title"), str(exc))
    ...
    def refresh_memory_table(self):
        ...
        for index, rule in enumerate(rules):
            ...
            self.memory_tree.insert("", "end", iid=str(index), tags=(tag,), values=(rule.get("match", ""), self.category_to_display(rule.get("category", "")), rule.get("note", "")))

        self.memory_tree.tag_configure("even", background="#ffffff")
        self.memory_tree.tag_configure("odd", background="#f8fafc")

        self.memory_count_var.set(t("memory.count", count=len(rules)) if get_language() == "en" else t("memory.count_zh", count=len(rules)))
        if self.memory_dirty:
            self._refresh_memory_status(t("memory.status_unsaved"))
        else:
            self._refresh_memory_status(t("memory.status_saved"))
    ...
    def _refresh_memory_status(self, text=None):
        ...
        if text:
            ...
            self.memory_status_var.set(t("memory.status_unsaved"))
        else:
            self.memory_status_var.set(t("memory.status_saved"))
    ...
    def edit_selected_memory_rule(self):
        ...
        if not indices:
            messagebox.showwarning(t("memory.none_selected_title"), t("memory.none_selected_message"))
            return
    ...
    def _open_memory_rule_editor(self, index):
        ...
        editor.title(t("memory.editor_new_title") if is_new else t("memory.editor_edit_title"))
        ...
        ctk.CTkLabel(card, text=t("memory.editor_new_title") if is_new else t("memory.editor_edit_title"), font=("Microsoft YaHei UI", 18, "bold"), text_color=COL_TEXT).pack(anchor="w", padx=20, pady=(18, 4))
        ctk.CTkLabel(card, text=t("memory.editor_subtitle"), font=("Microsoft YaHei UI", 11), text_color=COL_TEXT_MUTED).pack(anchor="w", padx=20, pady=(0, 12))

        ctk.CTkLabel(card, text=t("memory.editor_keyword"), anchor="w").pack(fill="x", padx=20, pady=(8, 4))
        match_var = tk.StringVar(value=rule.get("match", ""))
        ctk.CTkEntry(card, textvariable=match_var, height=36).pack(fill="x", padx=20)

        ctk.CTkLabel(card, text=t("memory.col_category"), anchor="w").pack(fill="x", padx=20, pady=(14, 4))
        ...
        ctk.CTkLabel(card, text=t("memory.editor_note"), anchor="w").pack(fill="x", padx=20, pady=(14, 4))
        ...
        def save():
            ...
            if not match_text:
                messagebox.showwarning(t("memory.missing_keyword_title"), t("memory.missing_keyword_message"), parent=editor)
                return
            new_rule = {"match": match_text, "category": self.display_to_category(cat_var.get()), "note": note_var.get().strip()}
            if is_new:
        ...
        ctk.CTkButton(action_row, text=t("memory.cancel"), fg_color="transparent", border_width=1, border_color=COL_BORDER, text_color=COL_TEXT_NAV, hover_color=COL_HOVER, height=38, command=editor.destroy).grid(row=0, column=0, padx=(0, 8), sticky="ew")
        ctk.CTkButton(action_row, text=t("memory.save_rule"), fg_color=COL_ACCENT, hover_color="#1d4ed8", height=38, command=save).grid(row=0, column=1, padx=(8, 0), sticky="ew")

    def delete_selected_memory_rules(self):
        indices = self.get_selected_memory_indices()
        if not indices:
            messagebox.showwarning(t("memory.none_selected_title"), t("memory.delete_none_message"))
            return
        if not messagebox.askyesno(t("memory.confirm_delete_title"), t("memory.confirm_delete_message", count=len(indices))):
            return
    ...
    def save_memory_panel(self, silent=False):
        try:
            ...
            self._refresh_memory_status(t("memory.status_saved"))
            if not silent:
                messagebox.showinfo(t("memory.saved_title"), t("memory.saved_message"))
            return True
        except Exception as exc:
            messagebox.showerror(t("memory.save_failed_title"), str(exc))
            return False
